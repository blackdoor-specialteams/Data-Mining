...
def nb_v1(table,attlist,keycol):
	"""runs NB with a NHTSA rating for weight"""
...
def nb_v2(table,attlist,keycol):
	"""Runs NB with a gaussian distance for weight"""
...
def temp_table_with_NHTSA_rating(table):
	"""rebuilds a table with NHTSA ratings"""
	tmp = table[:]
	for row in tmp:
...
def build_table_with_gaussian(table,col):
	"""rebuilds a table with gaussian values in a column"""
	tmp = table[:]
...
def s4_KNN(training, test):
	"""runs K-nn against a training set and a test set, then calculates the probablity and standard error for the predictions"""
	nnn = 5
...
def s4_LR(training,test):
	"""runs Linear Regression against a training set and a test set, then calculates the probablity and standard error for the predictions"""
	keycol = 1
	weights = []
	mpgs = []
	for row in training:
...
def run_NB(nbtable,test):
	"""runs Naive Bayes against a training set and a test set, then calculates the probablity and standard error for the predictions"""
	keycol = 1
	attlist = [2,3,4]
	rules = build_all_class_dicts(nbtable,keycol,attlist)
...
def calculate_stdE(p,t):
	"""calculates standard error for probablity"""
...
def s5_LR_dict(lrd,training,test):
	"""Uses Linear Regression to Predict MPG"""
	weights = []
	mpgs = []
	for row in training:
		weights.append(float(row[4]))
...
def s5_KNN_dict(thing, training, test):
	"""Runs Knn over the training and test sets"""
	nnn = 5
	training = table_to_lick_dicts(training)
	test = table_to_lick_dicts(test)
	act = []
...
def s5_NB1_dict(mac, training,test):
	"""Formats the table with NHTSA ratings and runs Naive Bayes"""
...
def s5_NB2_dict(mac, training,test):
	"""Formats the table with guassian distance and runs Naive Bayes"""
	nbtable = build_table_with_gaussian(training,4)
	ac, p = s5_run_NB(nbtable,test)
	update_cm(mac,ac,p)

def s5_run_NB(nbtable,test):
	"""runs Naive Bayes based on the inputed tables"""
	keycol = 1
	attlist = [2,3,4]
	rules = build_all_class_dicts(nbtable,keycol,attlist)
	rand_inst = get_random_table_indexes(nbtable,10,len(nbtable))
...
def s6_run_NB(nb,training,test):
	"""runs NB over a fold of the titanic set"""
	keycol = 3
	attlist = [0,1,2]
...
def s6_KNN_dict(thing, training, test):
	"""runs Knn over a fold of the titanic set"""
	nnn = 5
...
def get_list_of_att(att,ls):
	"""returns a list of a specic attribute from a list of dictionarys"""
	result = []
	for x in ls:
...
def get_counts_for_con(tb):
	"""gets the predidctions and actual values from a list of lists"""
	a = []
	p = []
	for x in tb:
		a.append(x[0])
